...
app.get("/user", async (req, res) => {
    ...
    try {
        ...
        res.status(500).send("Something went wrong while fetching user data."+ error.message);
    }
})
...
app.delete("/user", async (req, res) => {
    ...
    try {
        ...
        res.status(500).send("Something went wrong while deleting user data."+ error.message);
    }
})
...
app.patch("/user/:userId", async (req, res) => {
    ...
    try {
        const AllowedUpdates = ["firstName", "lastName", "skills", "password", "age", "photoUrl", "phoneNumber", "about",];
        ...
        if(updatedData.skills.length > 5) {
            throw new Error("Skills array should not exceed 5 items."+ error.message); 
        }
        ...
        res.status(500).send("Something went wrong while updating user data."+ error.message);
    }
})
